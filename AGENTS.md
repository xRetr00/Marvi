# Marvi Agent - Development Guide

Instructions for AI coding assistants and developers working on the Marvi-agent codebase.
...
Marvi is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Marvi ships a **lot** — most merges are bug fixes to
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.hermes/plugins/` (or via a
  pip entry point), and promote them in the NeuRetro Research Discord
...
Applies to TypeScript across Marvi: desktop, TUI, website, and future TS packages.
...
**Upstream sync rule:** Marvi carries downstream features that may not exist in upstream Hermes. During any upstream merge, preserve streaming STT (including Parakeet/Moonshine integration), wake word, voice presence/Dynamic Island, glow overlay, Qwen3-TTS, PocketTTS, voice-residency tiers, presence-aware resource policy, and backend supervision, together with their config keys, settings UI, Electron IPC/window wiring, desktop composer/status hooks, tests, and installer/updater integration. Marvi's only wake-word implementation is `voice.wake_word` with the "Hey Marvi" phrase/model path; do not restore upstream's standalone "Hey Hermes" listener, bundled `hey_hermes` models, top-level `wake_word` config, `/wake` command, `wake.*` gateway RPCs, or duplicate Desktop/TUI controls. Preserve the instant voice lane and duplex/speaker-focus behavior as an independent fast route, including its model selection, escalation/delegation, barge-in, speaker identity/self-enrollment, world-context, config/API/UI, and test contracts. Also preserve the subconscious, mind, and presence stack: goal store/tools, proactive tick and idle trigger, connected-account snapshots (GitHub, Gmail, Calendar, Slack), ActivityWatch/media/rhythm observation, distilled-memory viewer, narrative/reflection/initiative views, REST activation endpoints, CLI commands, and their Settings -> Presence/Subconscious/Mind UI. These settings must retain their activation paths: do not replace the subconscious/presence activation endpoints with raw config writes. Preserve local learning loops and their outcome, reflection, timing, focus-app, trust, escalation, voice-threshold, and room-habit signals, configuration, scheduled execution, suggestions, and tests.

Before changing branches, run `python scripts/prepare_marvi_upstream_sync.py --review-only`; use the generated protected-overlap report during resolution, then run `python scripts/verify_marvi_upstream_contract.py` and `python scripts/verify_marvi_brand.py` before committing.

Keep Desktop packaging branded and launchable: electron-builder must emit `Marvi.exe` / `Marvi.app` / `Marvi` and `Marvi-${version}-...` installer artifacts, while `hermes_cli/main.py` must resolve those names first after `hermes desktop --build-only`. Legacy Hermes artifact names may remain fallback-only for compatibility; never make them the sole resolver candidates.

Keep the `hermes_cli/web_server.py` contract for `/api/mind`, `/api/subconscious/*`, `/api/presence/*`, `/api/learning/*`, `/api/marvi/knowledge`, and `/api/brain/*`; these routes activate jobs, supply Mind data, and power the Presence/Subconscious UI. In the contribution desktop shell, retain the `/mind` workspace route and its sidebar entry, plus the Voice Presence toggle and provider-aware voice pipeline/warmup dots in the statusbar. Do not treat these as obsolete `desktop-controller` code during refactors.

Protect memory maturity end to end: `agent/memory` episodic SQLite/FTS storage, `recall_episode`, scheduler and distiller ingestion, reflection context, Mind Timeline, `/api/memory/episodes`, weekly dreaming, adaptive retrieval, decay/contamination checks, archive/restore APIs and Knowledge viewer. Keep its consent rule: memory-changing merges and contradiction resolution remain proposal-driven, while archival is reversible. Preserve the complete desktop voice contract in `hermes_cli/web_server.py`: streaming transcription and wake-word WebSockets, voice warmup/residency, duplex/instant-lane streaming, speaker enrollment/status APIs, PocketTTS playback captions/levels, and the related speaker-model and wake-word Settings UI.

Preserve Dynamic Island work state as a primary-window-to-island IPC path (`main.ts`, preload, `voice-island` store/app/work panel), including visibility while Marvi works outside an active voice turn. Preserve proactive suggestion polling/delivery and its notification/island handoff. The smart-room protection includes clap detection, bounded dataset persistence, review/clear APIs, desktop notifications and settings, normal/light/sleep modes, sound-event thresholds, and guest/owner greeting behavior; keep plugin runtime, dashboard API, tools, docs, and tests together.

Preserve the smart-room plugin and world/proactive integration: its plugin manifest and skill, Windows runtime/process supervision, secret/config boundaries, presence fusion (BLE, mmWave, OwnTracks/geofence), MQTT/Tuya integration, sound/clap events, modes, alarms, welcomes, dashboard/tools APIs, compact world snapshots/context injection, proactive delivery, and Desktop Settings UI. Preserve the related desktop navigation, types, localization, REST/IPC contracts, and tests whenever upstream rewrites nearby UI or backend files. NeuTTS and KittenTTS are intentionally blocked in Marvi; do not restore their setup options, installers, docs, or UI wiring from upstream. If upstream deletes or rewrites adjacent files, reapply these Marvi features on top of the upstream structure; do not drop them as conflict cleanup.
...
Preserve the August 2026 voice and Smart Room additions as one protected contract: instant-voice provider/model selection with curated fallback and hard cancellation of stalled calls; ordered phrase-streamed duplex TTS; PocketTTS 2.1 language, temperature, decode-step, quantization, device, and voice controls; and lazy streaming-provider discovery used by the standalone Gepard integration and its Desktop/API settings. Smart Room vision must retain its supervised camera service, independent pose/gesture and face inference cadence, reviewed local face enrollment (never automatic identity learning), unknown-face evidence, bounded visual history, sleep/zone/gesture reasoning, restricted cognition worker, world/proactive/subconscious integration, plugin tools and dashboard routes, camera preview, and Desktop camera/model/face/gesture settings. Preserve their tests and optional dependency boundaries; do not move camera or model dependencies into Marvi core.

Preserve the post-August 2026 voice and Smart Room recovery additions too. Configured PocketTTS 2.1 and LiveKit Hey Marvi dependencies must self-repair after managed updates prune optional packages; duplex STT permit cleanup must remain cancellation-safe; and the instant lane must retain conversational end-of-session judgment plus silent accidental-wake dismissal. Smart Room must keep managed runtime/vision dependency repair, MQTT reconnect telemetry, per-device Tuya reconnection and the direct probe script, contention-safe event-log trimming, the sleep-safe entry-light reflex, vision-driven sleep activation, the dedicated low-latency gesture and serialized action lanes, release/gap gating, and the reviewed pending-face queue. Face sampling stays bounded, duplicate-aware, and user-controlled, with per-item previews and individual/bulk accept or reject APIs and Desktop controls; it must never become automatic identity enrollment.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Marvi built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Marvi core.
...
core Marvi tool that should ship in the base system.
...
- `default` — Classic Marvi gold/kawaii (the current look)
...
Marvi has two plugin surfaces. Both live under `plugins/` in the repo so
...
don't own. Promote standalone plugins in the NeuRetro Research Discord
...
[`hermes-example-plugins`](https://github.com/NeuRetro/hermes-example-plugins)
...
2. **Tools referenced in SKILL.md prose must be native Marvi tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Marvi Agent" is the secondary collaborator. If the
   contributor's commit shows "Marvi Agent" as author (because they
   used Marvi to draft the skill), replace it with their actual name
...
Marvi-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
Marvi supports **profiles** — multiple fully isolated instances, each with its own
