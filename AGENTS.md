...
**Upstream sync rule:** Marvi carries desktop voice features that may not exist in upstream Hermes. During any upstream merge, preserve streaming STT, wake word, voice presence, glow overlay, Qwen3-TTS, PocketTTS, their settings, and their UI wiring. NeuTTS and KittenTTS are intentionally blocked in Marvi; do not restore their setup options, installers, docs, or UI wiring from upstream. If upstream deletes or rewrites adjacent desktop files, reapply these Marvi features on top of the upstream structure; do not drop them as conflict cleanup.
