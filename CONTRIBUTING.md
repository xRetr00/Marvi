# Contributing to Marvi Agent



Thank you for contributing to Marvi Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.

The legacy `hermes` command, `HERMES_*` env vars, and `~/.hermes` home directory remain supported for compatibility, so examples in this guide keep using them where they are part of the existing workflow.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Marvi to work everywhere.
...
Bundled skills (in `skills/`) ship with every Marvi install. They should be **broadly useful to most users**:
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [xRetro Labs Research community](https://github.com/xRetr00/Marvi). Users can install it with `hermes skills install`.
...
The installer creates the Marvi venv, wires the `hermes` compatibility command, stamps the
...
git clone https://github.com/xRetr00/Marvi.git
cd Marvi
...
├── website/                  # Documentation site (website/docs/)
│
...
The user may skip setup and keep loading the skill. Marvi only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
2. **Tools referenced in SKILL.md prose must be native Marvi tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Marvi Agent" is the secondary collaborator. If the contributor's commit shows "Marvi Agent" as author because they used Marvi to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Marvi tools (`web_extract`, `terminal`, `read_file`).
...
Marvi uses a data-driven skin system — no code changes needed to add a new skin.
...
Marvi runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
Marvi has terminal access. Security matters.
...
- Use [GitHub Issues](https://github.com/xRetr00/Marvi/issues)

- Include: OS, Python version, Marvi version (`hermes version`), full error traceback
...
- **Discord**: [github.com/xRetr00/Marvi](https://github.com/xRetr00/Marvi) — for questions, showcasing projects, and sharing skills
