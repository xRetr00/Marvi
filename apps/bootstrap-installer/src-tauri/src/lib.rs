...
/// desktop app handing off via `Marvi-Setup.exe --update`).
///
...
pub fn run() {
    ...
    // Marvi is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Marvi installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Marvi is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Marvi" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Marvi" shortcuts that install.ps1 creates, and a
        ...
        .expect("error while running Marvi Setup");
}
