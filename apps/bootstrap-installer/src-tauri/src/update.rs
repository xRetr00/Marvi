    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Marvi manually."),
            );
        }
    } else if let Err(err) =
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    ...
    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Marvi to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            emit_log(
                ...
                Some("update"),
                LogStream::Stdout,
                "[update] Marvi still holding the venv shim; force-killing stragglers…",
            );
