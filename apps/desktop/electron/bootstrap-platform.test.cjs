...
test('isWindowsBinaryPathInWsl blocks Windows binary types on WSL', () => {
  assert.equal(isWindowsBinaryPathInWsl('/mnt/c/Tools/marvi.exe', { isWsl: true }), true)
  ...
  assert.equal(isWindowsBinaryPathInWsl('/mnt/c/Tools/marvi.exe', { isWsl: false }), false)
})
