...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message: 'Updating Marvi — this window will close and the updater will open. Don’t reopen Marvi yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
        const handoffError = new Error('Marvi recovery was handed off to Marvi Setup. The desktop will restart when recovery completes.')
