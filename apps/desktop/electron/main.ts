...
let mainWindow = null
type SupervisedChildProcess = ReturnType<typeof spawn> & { __intentionalStop?: boolean }
const backendConnectionState = createBackendConnectionState<SupervisedChildProcess, any>()
...
// HERMES_HOME/marvi-setup.exe on a successful install (see
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'marvi-setup.exe' : 'marvi-setup'
...
// `marvi.exe` holds open; on POSIX it's never mandatory-locked.
function venvHermesShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'marvi.exe')
...
// only signals the direct child, so on Windows a backend `marvi.exe` that
...
// teardown, so the updater raced a still-locked `marvi.exe`, the quarantine
...
// REPL, a pty terminal, the gateway) keep `marvi.exe` and other files in the
...
// `hermes update`). Instead we spawn the staged Marvi Setup binary with
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri marvi-setup. Unlike Windows
    ...
    if (!updater) {
      ...
      // marvi-setup.exe into HERMES_HOME). They DO have a working `hermes`
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Marvi — this window will close and the updater will open. Don’t reopen Marvi yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // marvi.exe (held by the backend child / its grandchildren) and the update
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvMarvi = venvHermesShimPath(updateRoot)
...
async function applyUpdatesPosixInApp(opts: any) {
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Marvi (git + dependencies)…', percent: 10 })
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Marvi to load the new version.'
        }
      }
    }
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Marvi to finish.'
    }
  }
...
function resolveHermesBackend(backendArgs) {
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Marvi CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      rememberLog(
        `Ignoring existing Marvi CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  return {
    ...
    label: 'Marvi Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Marvi recovery was handed off to Marvi Setup. The desktop will restart when recovery completes.'
      )
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Marvi Cloud agent…' : 'Sign in to Marvi gateway',
        ...
        try {
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Marvi on the remote host before connecting with Desktop SSH.'
            }
          }
...
function createWindow() {
  ...
  mainWindow.once('ready-to-show', () => {
    if (mainWindow && !mainWindow.isDestroyed() && !START_HIDDEN) {
      mainWindow.show()
    }
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Marvi agent venv at ${VENV_ROOT}.`
    }
  }
