...
async function locateHermes(ssh, remoteHermesPath) {
  ...
  const err: any = new Error(
    'Marvi is not installed on the remote host (could not find a `hermes` compatibility executable). ' +
      'Install it on the remote with:  curl -fsSL https://raw.githubusercontent.com/xRetr00/Marvi/main/scripts/install.sh | sh  ' +
...
// e.g. "Marvi Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH hermes a
...
async function spawnRemoteDashboard(ssh, { hermesPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, hermesPath))) {
    const err: any = new Error(
      'The remote Marvi install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Marvi on the remote host to continue using Desktop SSH mode.'
    )
