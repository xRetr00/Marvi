#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Marvi icon + version metadata onto the
// built Marvi.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Marvi.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-Marvi.exe>
//
...
// Stamp the Marvi icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  console.log('[set-exe-identity] done — Marvi icon + identity stamped')
}
