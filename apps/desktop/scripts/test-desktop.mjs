...
// no longer bundles the Marvi Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Marvi.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Marvi'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Marvi.exe'),
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Marvi-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Marvi-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Marvi-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Marvi-${version}-${os}-${arch}.${ext}'
...
//   - The Marvi Agent Python payload is NOT shipped (it's fetched at first
