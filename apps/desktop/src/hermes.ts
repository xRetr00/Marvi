...
export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Marvi gateway connection closed',
      connectErrorMessage: 'Could not connect to Marvi gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Marvi gateway is not connected',
...
export function getMcpCatalog(
  optsOrProfile: { online?: boolean; profile?: null | string; query?: string } | null | string = {}
): Promise<McpCatalogResponse> {
  const opts = typeof optsOrProfile === 'object' && optsOrProfile !== null ? optsOrProfile : {}
  const profile = typeof optsOrProfile === 'string' || optsOrProfile === null ? optsOrProfile : opts.profile
  const params = new URLSearchParams()
...
export function installMcpCatalogEntry(
  name: string,
  env: Record<string, string> = {},
  entryOrProfile?: Pick<McpCatalogEntry, 'registry_id' | 'source_kind'> | null | string,
