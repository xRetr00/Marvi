...
export const ja = defineLocale({
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Marvi Desktop を再起動してください。',
    ...
    native: {
      ...
      rejectAction: '拒否',
      inputTitle: '入力が必要です',
      inputBody: 'Marvi が応答を待っています。',
      turnDoneTitle: 'Marvi が完了しました',
  ...
  settings: {
    ...
    notifications: {
      ...
      focusedHint: '完了通知は Marvi がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'Marvi が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'Marvi がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
  ...
  updates: {
    ...
    applyingBody:
      'Marvi アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に Marvi を再度開きます。更新中はご自分で Marvi を開き直さないでください。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると Marvi が自動的に再接続します。',
    applyingClose: 'このウィンドウは更新中に閉じ、その後 Marvi が自動的に再度開きます。',
  ...
  install: {
    ...
    settingUpTitle: 'Marvi Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
  ...
  shell: {
    ...
    statusbar: {
      ...
      desktopVersion: version => `Marvi Desktop v${version}`,
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Marvi Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには Marvi Desktop を再起動してください',
