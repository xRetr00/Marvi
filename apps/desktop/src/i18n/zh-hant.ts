...
export const zhHant = defineLocale({
  ...
  settings: {
    ...
    quickEntry: {
      ...
      placeholder: '向 Marvi 提問…',
      disconnected: '開啟 Marvi 以重新連線',
      sendTo: '傳送至',
      currentChat: '目前聊天',
      newSession: '新工作階段'
    },
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'Marvi Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 Hermes 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
      appliesTo: '套用至',
      allProfiles: '全部設定檔',
      ...
      sshErrNotInstalled:
        '遠端主機上未安裝 Marvi。請在遠端安裝（curl -fsSL https://raw.githubusercontent.com/xRetr00/Marvi/main/scripts/install.sh | sh）或設定 Marvi 路徑。',
      sshErrPlatform: '不支援的遠端平台。Marvi Desktop 的 SSH 模式支援 Linux、macOS 和 Windows 遠端主機。',
  ...
  mind: {
    ...
    graph: {
      title: '圖譜',
      nodeCount: count => `${count} 個節點`,
      description: '探索 Marvi 如何連結人物、專案、事實和事件。選取節點即可檢視或修改。',
      searchPlaceholder: '搜尋或聚焦節點',
      filterLabel: '篩選節點類型',
      allTypes: '所有類型',
      types: {
        person: '人物',
      ...
      unavailable: '後端離線時無法使用圖譜。',
      empty: '隨著 Marvi 將所學內容建立連結，心智圖譜會逐漸形成。',
      canvasLabel: '知識圖譜',
      selectHint: '選取節點以檢視摘要、來源、連結和可用操作。',
      connections: '連結',
      noConnections: '尚無已記錄的連結。',
      connectionsFailed: '無法載入節點連結',
      salience: value => `顯著度 ${value}%`,
      source: kind => `來自 ${kind}`,
      edit: '編輯',
      delete: '刪除',
      ...
      deleteDescription: '這會從圖譜中移除該節點及其連結，但不會變更其來源記憶。'
    },
    composio: {
      title: '已連接應用程式',
