...
from hermes_constants import PRODUCT_NAME, get_hermes_home
...
_UPSTREAM_REPO_URL = "https://github.com/xRetr00/Marvi"
_OFFICIAL_REPO_CANONICAL = "github.com/xretr00/marvi"
...
_RELEASE_URL_BASE = "https://github.com/xRetr00/Marvi/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical xRetr00/Marvi repo (forks don't get a link).
    """
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]NeuRetro Labs Research[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]NeuRetro Labs Research[/]")
