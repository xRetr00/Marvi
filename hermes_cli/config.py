...
PLATFORM_SUPPORT_DOCS_URL = "https://github.com/xRetr00/Marvi/docs/getting-started/platform-support"
...
DEFAULT_CONFIG = {
    ...
    "tts": {
        "provider": "pockettts",  # local | edge | elevenlabs | openai | xai | minimax | mistral | gemini | deepinfra | piper
        "edge": {
        ...
        # NeuTTS and KittenTTS are intentionally blocked in Marvi (see
        # hermes_cli/setup.py's tool-status check and hermes_cli/tips.py) —
        # these defaults stay only so config schemas/imports from upstream
        # Hermes configs don't break; the setup wizard steers users to
        # PocketTTS instead of ever wiring these up.
        "kittentts": {
    ...
    "cron": {
        ...
        # Keep delivered cron responses conversational. Set to true only when
        # the legacy diagnostic header/footer is useful.
        "wrap_response": False,
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://github.com/xRetr00/Marvi/docs/api/model-catalog.json",
...
OPTIONAL_ENV_VARS = {
    ...
    "LLAMACPP_API_KEY": {
        "description": "llama.cpp bearer token for auth-enabled local servers",
        "prompt": "llama.cpp API key / bearer token",
        "url": None,
        "password": True,
        "category": "provider",
        "advanced": True,
    ...
    "LLAMACPP_BASE_URL": {
        "description": "llama.cpp base URL override",
        "prompt": "llama.cpp base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
