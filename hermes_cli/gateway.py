...
SERVICE_DESCRIPTION = "Marvi Agent Gateway - Messaging Platform Integration"
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    root's home.  This translates it to the target user's equivalent path:
      /root/.marvi                     → /home/alice/.marvi
      /root/.hermes                    → /home/alice/.hermes
      /root/.hermes/profiles/coder     → /home/alice/.hermes/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_home_raw = os.environ.get("HERMES_HOME", "").strip()
    current_home = (
        Path(current_home_raw).expanduser()
        if current_home_raw
        else get_hermes_home().expanduser()
    )
    ...
    for dirname in (".marvi", ".hermes"):
        current_default = calling_home / dirname
        target_default = Path(target_home_dir) / dirname
        if current_home == current_default:
            return str(target_default)
        try:
            relative = current_home.relative_to(current_default)
