...
Usage:
    ...
    hermes uninstall           Uninstall Marvi Agent
...
def _print_fast_version_info() -> None:
    ...
    print(f"Marvi Agent v{__version__} ({__release_date__})")
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with 'Marvi Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_uninstall(args):
    """Uninstall Marvi Agent (or just the Chat GUI with --gui)."""
...
def _update_via_zip(args):
    """Update Marvi Agent by downloading a ZIP archive.
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/hermes-agent.git",  # OFFICIAL_REPO_URL
    "git@github.com:NousResearch/hermes-agent.git",
    "https://github.com/NousResearch/hermes-agent",  # OFFICIAL_REPO_URL
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Marvi Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def cmd_update(args):
    """Update Marvi Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Marvi Agent")
        return
...
def cmd_acp(args):
    """Launch Marvi Agent as an ACP server."""
    try:
...
def main():
    ...
    # for one release. The Marvi-Setup.exe success screen tells users to
