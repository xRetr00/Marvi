...
def _set_process_title() -> None:
    ...
    try:
        ...
        setproctitle.setproctitle("marvi")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"marvi", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("MARVI_HOME") or os.environ.get("HERMES_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            marvi_cfg = os.path.join(os.path.expanduser("~"), ".marvi", "config.yaml")
            legacy_cfg = os.path.join(os.path.expanduser("~"), ".hermes", "config.yaml")
            cfg_path = marvi_cfg if os.path.exists(marvi_cfg) else legacy_cfg
...
def _print_fast_version_info() -> None:
    ...
    print(f"Marvi v{__version__} ({__release_date__})")
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with 'Marvi'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Marvi URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def cmd_login(args):
    """Authenticate Marvi CLI with a provider."""
...
def cmd_uninstall(args):
    """Uninstall Marvi (or just the Chat GUI with --gui)."""
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = [
            *release_dir.glob("mac*/Marvi.app/Contents/MacOS/Marvi"),
            *release_dir.glob("mac*/Hermes.app/Contents/MacOS/Hermes"),
        ]
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Marvi.exe",
            release_dir / "win-ia32-unpacked" / "Marvi.exe",
            release_dir / "win-arm64-unpacked" / "Marvi.exe",
        ...
        candidates = [
            release_dir / "linux-unpacked" / "marvi",
            release_dir / "linux-unpacked" / "Marvi",
            release_dir / "linux-arm64-unpacked" / "marvi",
            release_dir / "linux-arm64-unpacked" / "Marvi",
...
def _update_via_zip(args):
    """Update Marvi by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/xRetr00/Marvi/archive/refs/heads/{branch}.zip"
    )
...
OFFICIAL_REPO_URLS = {
    "https://github.com/xRetr00/Marvi.git",
    "git@github.com:xRetr00/Marvi.git",
    "https://github.com/xRetr00/Marvi",
    "git@github.com:xRetr00/Marvi",
}
OFFICIAL_REPO_URL = "https://github.com/xRetr00/Marvi.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from xRetr00/Marvi.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/xRetr00/Marvi.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/xRetr00/Marvi.git' to add later."
            )
...
def _quarantine_running_hermes_exe(
    ...
    handles (recovers in <1s), or the Marvi Desktop backend child process
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Marvi — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    for pid, name, cmdline in matches[:6]:
        ...
        if "serve" in low or "dashboard" in low:
            hint = "  ← Marvi Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
...
def cmd_update(args):
    """Update Marvi to the latest version.
...
def _cmd_update_pip(args):
    """Update Marvi via pip (for PyPI installs)."""
...
def main():
    ...
    # for one release. The Marvi Setup success screen tells users to
