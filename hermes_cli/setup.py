...
_DOCS_BASE = "https://github.com/xRetr00/Marvi"
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("Marvi Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Marvi runs shell commands and code.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Marvi delivers cron job results,")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/xRetr00/Marvi")
    print()
    ...
    print_info("   https://github.com/xRetr00/Marvi#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Marvi from anywhere.")
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color("│     Marvi Setup — Nous Portal (one-shot)                │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     Marvi Setup — {label:<36s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
