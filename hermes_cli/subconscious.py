"""``hermes subconscious`` command implementation.
...
def subconscious_command(args) -> int:
    """Handle ``hermes subconscious <enable|disable|status>``."""
    ...
    print("Usage: hermes subconscious [enable|disable|status]", file=sys.stderr)
    return 1
