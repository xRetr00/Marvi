...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Marvi-Setup.exe
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Marvi-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        if ($env:OS -eq "Windows_NT") {
            ...
            # The launcher CLI shims plus their child trees.
            & taskkill /F /T /IM marvi.exe /FI "PID ne $myPid" 2>$null | Out-Null
            & taskkill /F /T /IM hermes.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM marvi.exe or hermes.exe is NOT enough: the gateway/agent that a
