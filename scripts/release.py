...
def _update_npm_versions(semver: str) -> None:
    """Keep root, desktop, and lockfile workspace metadata in lockstep."""
    for relative_path in ("package.json", "apps/desktop/package.json"):
    ...
    if lock_path.exists():
        ...
        lock["version"] = semver
        for workspace in ("", "apps/desktop"):
            package = lock.get("packages", {}).get(workspace)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"marvi-agent[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/xRetr00/Marvi",
    ...
    lines.append(f"# Marvi Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Marvi Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  Marvi Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Marvi Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Marvi Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Marvi Agent v{new_version} ({calver_date})' "
