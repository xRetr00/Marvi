...
author: Real Name (github-handle), Marvi Agent
license: MIT
...
- Credit the **human first**, then "Marvi Agent" as secondary collaborator: `Ben Barclay (benbarclay), Marvi Agent`.
- Never `author: Marvi Agent` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
