...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    desktop_dir = root / "apps" / "desktop"
    if platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Marvi.app" / "Contents" / "MacOS" / "Marvi"
    elif platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Marvi.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "Marvi"
