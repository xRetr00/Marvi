...
class TestWebServerEndpoints:
    ...
    def test_config_schema_excludes_blocked_tts_provider_names(self):
        from hermes_cli.config import load_config, save_config

        cfg = load_config()
        tts = cfg.setdefault("tts", {})
        tts["provider"] = "neutts"
        tts.setdefault("providers", {})["kittentts"] = {
            "type": "command",
            "command": "blocked {text}",
        }
        save_config(cfg)

        lowered = [o.lower() for o in self._schema_provider_options("tts.provider")]
        assert "neutts" not in lowered
        assert "kittentts" not in lowered
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://github.com/xRetr00/Marvi/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        assert teams["docs_url"] == (
            "https://github.com/xRetr00/Marvi/docs/user-guide/messaging/teams"
        )
    ...
    def test_google_chat_messaging_metadata_links_setup_guide(self):
        ...
        assert google_chat["docs_url"] == (
            "https://github.com/xRetr00/Marvi/docs/user-guide/messaging/google_chat"
        )
...
class TestNewEndpoints:
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        # NeuTTS and KittenTTS are intentionally blocked in Marvi (not listed
        # in TOOL_CATEGORIES["tts"]["providers"]) — only Piper is offered as
        # a keyless local row here, so it's the only "needs_setup" check.
        assert "KittenTTS" not in by_name
