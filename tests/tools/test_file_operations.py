...
class TestSearchFilesFallbackHiddenPaths:
    ...
    @pytest.mark.skipif(os.name == "nt", reason="POSIX find fallback is unavailable on Windows")
    ...
    @pytest.mark.skipif(os.name != "nt", reason="Windows-specific find.exe guard")
    def test_windows_find_exe_is_not_used_as_posix_fallback(self, monkeypatch):
        ops = ShellFileOperations(self._make_env())
        monkeypatch.setattr(ops, "_has_command", lambda command: command == "find")

        result = ops._search_files("*.log", ".", limit=50, offset=0)
