...
class TestTextToSpeechToolWithCommandProvider:
    @pytest.mark.parametrize("provider", ["neutts", "kittentts"])
    def test_blocked_provider_cannot_be_restored_as_custom_command(self, provider):
        cfg = {
            "provider": provider,
            "providers": {
                provider: {"type": "command", "command": "echo should-not-run"},
            },
        }
        with patch("tools.tts_tool._load_tts_config", return_value=cfg):
            result = text_to_speech_tool(text="hi")
        data = json.loads(result)
        assert data["success"] is False
        assert "blocked in Marvi" in data["error"]
