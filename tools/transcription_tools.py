...
def _load_local_whisper_model(
    ...
    if device is None or compute_type is None:
        configured_device, configured_compute_type = _local_whisper_runtime_config()
        device = device or configured_device
        compute_type = compute_type or configured_compute_type
...
def _transcribe_local(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        model = _local_model
        if model is None or _local_model_name != model_key:
            with _local_model_lock:
