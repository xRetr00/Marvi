...
@register("pockettts")
class PocketTTSStreamer(StreamingTTSProvider):
    """Adapt Marvi's local PocketTTS chunks to the shared int16 PCM stream."""
    ...
    def __init__(self, tts_config: Dict, section: Dict):
        ...
        from tools.tts_tool import _pockettts_sample_rate

        self.sample_rate = int(_pockettts_sample_rate(tts_config))

    def stream(self, text: str) -> Iterator[bytes]:
        import numpy as np

        from tools.tts_tool import _stream_pockettts_audio
