        ...
        try:
            while True:
                if file_descriptor is None:
                    ...
                    chunk = data.decode(encoding, errors="replace") if data else ""
                if not chunk:
                    break
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the caller requested those formats.
    ...
    # WAV -- this keeps the tool usable on
...
def _resolve_piper_voice(tts_config: Dict[str, Any]) -> Any:
    ...
    with _piper_voice_cache_lock:
        return _tts_cache_get_or_load(
            _piper_voice_cache, cache_key, _load_piper_voice
        )
...
def _generate_piper_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # to 0 (Piper's own default). Booleans are rejected outright because
    # True/False would silently coerce to 1/0 and hide a config mistake.
...
def _text_to_speech_single(
    ...
    *,
    ...
    delivery packing, and post-encoding size enforcement. ``provider`` may
    override the configured built-in, command, or plugin provider for this
    request.
    """
    ...
    if provider in BLOCKED_TTS_PROVIDERS:
        return tool_error(
            f"{provider} is blocked in Marvi; use PocketTTS or Piper",
            success=False,
        )
    ...
    if output_path:
        ...
        filename_stem = f"tts_{timestamp}_{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        # Providers may output MP3 or WAV. Keep their native
...
def check_tts_requirements() -> bool:
    ...
    if provider == "pockettts":
        return _check_pockettts_available()
        ...
        "properties": {
            ...
            "provider": {
                ...
                "description": (
                    ...
                    "piper, pockettts, deepinfra), user-declared command provider "
