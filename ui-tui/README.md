...
React + Ink terminal UI for Marvi. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
The CLI expects `ui-tui/dist/entry.js` to exist, or the whole source code available in which to run `npm install` and `npm run dev`. Legacy `hermes --tui` still works on existing installs.
