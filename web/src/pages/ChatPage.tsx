/**
 * ChatPage — embeds `marvi --tui` inside the dashboard.
 *
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__HERMES_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `marvi dashboard`, not directly. Legacy `hermes dashboard` still works."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner Marvi TUI has enabled xterm mouse-events
    ...
    //      `marvi --tui` (legacy `hermes --tui` still works).
    //
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[marvi-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into Marvi. Keyboard input, paste, and resize still
    // behave normally.
